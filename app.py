...
def visual(filename):
    ...
    df_melted = df_filtered.melt(
        id_vars=["timestamp"], var_name="Channel", value_name="Amplitude"
    )
    ...
    eeg_chart = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Channel",
                scale=alt.Scale(
                    domain=eeg_columns, range=[colors[ch] for ch in eeg_columns]
                ),
                legend=alt.Legend(
                    title="EEG Channels", titleFontSize=16, labelFontSize=14
                ),
            ),
    ...
    seizure_chart = (
        alt.Chart(df_filtered[df_filtered["predicted_class"] == 1])
        .mark_rule(color="pink", opacity=0.075)
        .encode(x="timestamp", size=alt.value(2))
    )
